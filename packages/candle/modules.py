...
class NoInputSpec(nn.Module):
    """Used to create a Module without specifying its number of inputs."""
    ...
    def _parameters(self):
        if self._parameters_not_specified:
            raise RuntimeError('Module has not yet been called on example inputs, so it is not yet fully specified.')
        else:
